...
(() => {
  ...
  function renderValidation() {
    ...
    const rows = [
      ...
      ["V-formation benefit", "trailing L/D > isolated", DATA.birdsFormation ? `sign ${DATA.birdsFormation.benefit_sign_positive ? "positive" : "not resolved"}: trailing L/D ${DATA.birdsFormation.LD.trail_right.toFixed(3)}/${DATA.birdsFormation.LD.trail_left.toFixed(3)} vs isolated ${DATA.birdsFormation.LD.isolated.toFixed(3)} (+${((DATA.birdsFormation.LD.trail_right / DATA.birdsFormation.LD.isolated - 1) * 100).toFixed(1)}%)` : "3D run pending", DATA.birdsFormation ? (DATA.birdsFormation.benefit_sign_positive ? "PASS" : "NOTE") : "…", "Portugal et al. 2014"],
